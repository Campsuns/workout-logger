...
let state = {{
  ...
  exercises: [], splits: [], logs: [], byId: {{}}, 
  currentSplit: '', period: 'week', eq: '', mg: '', page: 'list'
}};
...
// Refresh data periodically / on resume
...
// Prevent double-tap zoom inside the log modal only
(function(){{ let last=0; document.addEventListener('touchend', e=>{{ if(!$('#logModal')?.open) return; const now=Date.now(); if(now-last<300) e.preventDefault(); last=now; }}, {{passive:false}}); }})();
...
function equipDisplay(lbl){{ const s=String(lbl||'').trim(); if(!s) return 'Other'; return s.replace(/\b\w/g,c=>c.toUpperCase()).replace(/\bEz\b/g,'EZ').replace(/\bV Bar\b/g,'V-Bar'); }}
...
// Users / scoping
function logsForUser(){{ const uid=state.userId; if(uid==='all') return state.logs; return state.logs.filter(l => (l.user_id||'u_camp')===uid); }}
function exercisesForUser(){{ const uid=state.userId; return state.exercises.filter(e=>{{ const owner=(e.owner||'all').toLowerCase(); return uid==='all' ? true : (owner==='all' || owner===uid); }}); }}
function renderUserSwitcher(){{ const sel=$('#userSelect'); if(!sel) return; const opts=[{{user_id:'all', name:'All'}}, ...state.users]; sel.innerHTML=opts.map(u=>`<option value="${{u.user_id}}">${{u.name}}</option>`).join(''); const saved=store.get().userId||'u_camp'; state.userId=saved; sel.value=saved; sel.onchange=()=>{{ state.userId=sel.value; store.set({{userId: state.userId}}); renderList(); renderSummary(); }}; }}

// Latest log per exercise/side (user-aware, ignore skip_progress)
function isSkip(l){{ const v=String(l.skip_progress??'').toLowerCase(); return v==='1'||v==='true'||v==='yes'; }}
function latestFor(exId, side){{ const uid = state.userId==='all'?null:state.userId; const want=String(side||'both').toLowerCase(); const norm=x=>String(x||'both').toLowerCase(); const valid=state.logs.filter(l=>l.exercise_id===exId && (!uid || (l.user_id||'u_camp')===uid) && !isSkip(l)); if(!valid.length) return null; const same=valid.filter(l=>norm(l.side)===want); const arr = same.length ? same : valid; arr.sort((a,b)=>new Date(b.timestamp||b.date)-new Date(a.timestamp||a.date)); return arr[0]||null; }}

// Suggestion engine (rep-first 6–12, then +5 lb; gentle deload)
function suggestNext(exId, side){{
  const ex=state.byId[exId]; if(!ex) return {{weight:Number(ex?.default_weight||0), reps:8, sets:Number(ex?.sets||3), height:Number(ex?.default_height||0)}};
  const last=latestFor(exId, side); if(!last) return {{weight:Number(ex.default_weight||0), reps:8, sets:Number(ex.sets||3), height:Number(ex.default_height||0)}};
  let reps=Number(last.planned_reps||8), wt=Number(last.weight_lb||ex.default_weight||0), h=Number(last.height||ex.default_height||0);
  const fail=Number(last.fail_reps||reps), rpe=Number(last.rpe_set2||8);
  if(fail>=reps && reps<CONFIG.REP_MAX && rpe<=8) reps++;
  else if(reps>=CONFIG.REP_MAX && fail>=reps-1){{ wt += CONFIG.DEFAULT_INC_LB; reps = Math.max(CONFIG.REP_MIN, 8); }}
  if(fail<=Math.max(CONFIG.REP_MIN-1, reps-3) || rpe>=9) reps = Math.max(CONFIG.REP_MIN, reps-1);
...
function rankImprovements(period){{
  ...
  const byEx=new Map();
  for(const l of logs){{ const t=new Date(l.timestamp||l.date); const v=e1rm(l.weight_lb||0,l.planned_reps||0); const a=byEx.get(l.exercise_id)||[]; a.push({{t,v}}); byEx.set(l.exercise_id,a); }}
  const out=[]; const start=startOfPeriod(period), now=new Date();
  for(const [id,arr] of byEx.entries()){{ arr.sort((a,b)=>a.t-b.t); const startVal=(arr.find(x=>x.t>=start)||arr[0]||{{}}).v||0; const endVal=(arr.filter(x=>x.t<=now).slice(-1)[0]||{{}}).v||0; out.push({{id, delta:endVal-startVal}}); }}
  out.sort((a,b)=>b.delta-a.delta); return out;
}}
function streaks(allLogs){{
  // logs per week with >=3 sessions count toward streak
  const weeks = new Map();
  for(const l of allLogs){{ const d=new Date(l.date||l.timestamp); const key=d.getFullYear()+'-'+Math.ceil((((d - new Date(d.getFullYear(),0,1))/86400000)+1)/7); weeks.set(key, (weeks.get(key)||0)+1); }}
  const keys=[...weeks.keys()].sort();
  let cur=0,best=0,perfect=0, prev=null;
  for(const k of keys){{ const v=weeks.get(k); const wk=parseInt(k.split('-')[1],10); if(prev===null || wk===prev+1) cur++; else cur=1; if(v>=5) perfect++; if(cur>best) best=cur; prev=wk; }}
  return {{current:cur, best, perfectWeeks:perfect}};
}}

// API
async function apiGet(params){{ const url=new URL(CONFIG.API_URL); url.searchParams.set('token', CONFIG.TOKEN); Object.entries(params||{{}}).forEach(([k,v])=>url.searchParams.set(k,v)); const res=await fetch(url.toString(),{{method:'GET'}}); return res.json(); }}
async function apiPost(action, payload){{ const url=new URL(CONFIG.API_URL); url.searchParams.set('token', CONFIG.TOKEN); url.searchParams.set('action', action); const res=await fetch(url.toString(),{{method:'POST', body: JSON.stringify(payload)}}); return res.json(); }}
...
async function fetchAll(){{ 
  ...
  renderUserSwitcher(); renderFilters(); renderList(); renderSummary();
}}

// Restore UI
$$('[data-nav]').forEach(btn=>btn.addEventListener('click',()=>{{ const page=btn.dataset.nav; $$('[data-nav]').forEach(b=>b.classList.remove('active')); btn.classList.add('active'); state.page=page; store.set({{page}}); $$('.page').forEach(p=>p.classList.remove('active')); $('#page-'+page).classList.add('active'); }}));
(function restoreUI(){{ const s=store.get(); if(s.page){{ state.page=s.page; $$(`[data-nav="${{state.page}}"]`)?.classList.add('active'); $$('.page').forEach(p=>p.classList.remove('active')); $('#page-'+state.page).classList.add('active'); }} if(s.split!==undefined) state.currentSplit=s.split; if(s.period) state.period=s.period; if(s.eq!==undefined) state.eq=s.eq; if(s.mg!==undefined) state.mg=s.mg; if(s.userId) state.userId=s.userId; }})();
...
function renderFilters(){{
  ...
  const equipmentsRaw=[...new Set(items.map(e=>String(e.equipment||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
  const equipmentOptions=equipmentsRaw.map(lbl=>{{ const val=lbl.toLowerCase(); const display=lbl.replace(/\b\w/g,c=>c.toUpperCase()).replace(/\bEz\b/g,'EZ').replace(/\bV Bar\b/g,'V-Bar'); return {{value:val,label:display}}; }});
  $('#equipmentFilter').innerHTML = `<option value="">Equipment</option>` + equipmentOptions.map(o=>`<option value="${{o.value}}">${{o.label}}</option>`).join('');
...
function groupByEquipmentHTML(arr){{
  ...
  return keys.map(k=>`<div class="subgroup"><div class="group-title">${{k}}</div><div class="cards">${{map.get(k).map(makeCardHTML).join('')}}</div></div>`).join('');
}}
...
function renderList(){{
  ...
  if(state.currentSplit){{
    const splitRows=state.splits.filter(s=>String(s.split_name||'')===state.currentSplit && (!state.userId || !s.user_id || s.user_id===state.userId)).sort((a,b)=>Number(a.order||0)-Number(b.order||0));
  ...
  if(!items.length) otherList.innerHTML = `<div class="meta" style="padding:8px 4px;">No exercises match the current filters.</div>`;
}}
...
function openLog(exId){{
  ...
  $$('#sideSeg button').forEach(b=>b.classList.toggle('active', b.dataset.side===side));
...
function updateSteppers(){{
  $$('.stack .stepper').forEach(node=>{{ const field=node.dataset.field; const span=node.querySelector('.ctr span'); if(field&&span) span.textContent=fmt(stepVals[field]); }});
}}
...
function bindStepper(node){{
  ...
  const render=()=>{{ span.textContent=fmt(stepVals[field]); }};
  node.querySelector('.up').onclick = ()=>{{ stepVals[field]=Number(stepVals[field]||0)+step; render(); }};
  node.querySelector('.down').onclick = ()=>{{ stepVals[field]=Math.max(0, Number(stepVals[field]||0)-step); render(); }};
}}

$$('.stack .stepper').forEach(bindStepper);
$('#sideSeg').addEventListener('click', (e)=>{{ if(e.target.closest('button')){{ const b=e.target.closest('button'); $$('#sideSeg button').forEach(x=>x.classList.remove('active')); b.classList.add('active'); stepVals.side=b.dataset.side; saveSide(curEx.id, stepVals.side); const s=suggestNext(curEx.id, stepVals.side); stepVals.weight_lb=s.weight; stepVals.planned_reps=s.reps; stepVals.height=s.height; updateSteppers(); }} }});
  ...
  try{{ 
    ...
    if(res && !res.error){{
      if(sideUsed!=='both' && !modal.dataset.didOther){{
        ...
        $$('#sideSeg button').forEach(b=>b.classList.toggle('active', b.dataset.side===nextSide));
        stepVals.side = nextSide; saveSide(curEx.id, nextSide);
        const s2 = suggestNext(curEx.id, nextSide); stepVals.weight_lb=s2.weight; stepVals.planned_reps=s2.reps; stepVals.height=s2.height; updateSteppers();
        toast(`Logged ${{sideUsed}}. Now ${{nextSide}}.`);
        $('#btnLog').disabled=false; $('#btnSkip').disabled=false;
        fetchAll(); 
        return;
      }}
      modal.dataset.didOther=''; modal.close(); flash(); markDone(curEx.id); fetchAll();
    }} else {{ toast('Error: '+(res?.error||'unknown')); }}
  }} catch(err){{ toast('Network error'); }}
  finally{{ $('#btnLog').disabled=false; $('#btnSkip').disabled=false; }}
}}
...
async function saveExercise(){{
  const btn=$('#wizSave'); if(btn.dataset.busy==='1') return; btn.dataset.busy='1'; btn.disabled=true; const orig=btn.textContent; btn.textContent='Saving…';
  const body={{
...
function musclePercents(period){{
  const logs = logsForUser().filter(l=>inPeriod(l.date||l.timestamp, period));
  const score = new Map();
  for(const l of logs){{ const ex=state.byId[l.exercise_id]; if(!ex) continue; for(const slot of ['primary','secondary','tertiary']){{ const m=(ex[slot]||'').trim(); if(!m) continue; score.set(m, (score.get(m)||0) + MUSCLE_POINTS[slot]); }} }}
...
function renderSummary(){{
  ...
  const pct = musclePercents(period); const entries=Object.entries(pct).sort((a,b)=>b[1]-a[1]); const top=entries.slice(0,3); const neglected=entries.slice(-5);
